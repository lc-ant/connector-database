...
public abstract class AbstractDatabaseConnectorTest {
	...
	@Order(100001)
	void textSearch() {
		var e1 = db.create(EntityWithTextSearch.builder().title("Hello World!").content("Bonjour le monde!").nonIndexed("¡Hola Mundo!").build()).block();
		var e2 = db.create(EntityWithTextSearch.builder().title("le monde est vaste").content("This is a content").nonIndexed("This is not indexed").build()).block();
		var e3 = db.create(EntityWithTextSearch.builder().title("third").content("Entity number three").nonIndexed("Hello World").build()).block();
		var e4 = db.create(EntityWithTextSearch.builder().title("el mundo es grande").content("This is a content").nonIndexed("Hello World").build()).block();
		var e5 = db.create(EntityWithTextSearch.builder().title("une onde").content("").nonIndexed("Hello World").build()).block();
		
		var page = db.textSearch(EntityWithTextSearch.class, "monde", Optional.empty()).block();
		assertThat(page.getData())
			.containsExactlyInAnyOrder(e1, e2);

		page = db.textSearch(EntityWithTextSearch.class, "world", Optional.empty()).block();
		assertThat(page.getData())
			.containsExactly(e1);

		page = db.textSearch(EntityWithTextSearch.class, "nde", Optional.empty()).block();
		assertThat(page.getData())
			.containsExactlyInAnyOrder(e1, e2, e4, e5);
		
		/*
		page = db.textSearch(EntityWithTextSearch.class, "mundo number", Optional.empty()).block();
		assertThat(page.getData())
			.hasSize(2)
			.containsExactlyInAnyOrder(e3, e4);
		
		page = db.textSearch(EntityWithTextSearch.class, "this world content", Optional.empty()).block();
		assertThat(page.getData())
			.hasSize(3)
