...
public abstract class R2dbcConnector extends DatabaseConnector implements InitializingBean {
	...
	private <T> SelectQuery buildSelect(FindRequest<T> request, EntityMeta meta, String tableName, Collection<org.springframework.data.relational.core.sql.Expression> projection) {
		...
		query.setQuery(buildSelect(request, meta, tableName, projection, query).build());
		return query;
	}
	
	private <T> BuildSelect buildSelect(FindRequest<T> request, EntityMeta meta, String tableName, Collection<org.springframework.data.relational.core.sql.Expression> projection, SqlQuery<?> query) {
		...
		Optional<Condition> c = request.getWhere().flatMap(where -> extendedDialect.createCriteriaBuilder(meta, tableName, query).build(where));
		
		if (c.isPresent())
			queryBuilder = ((SelectWhere) queryBuilder).where(c.get());
	...
	private <T> SelectQuery buildCount(FindRequest<T> request, EntityMeta meta, String tableName) {
		...
		if (c.isPresent())
			countBuilder = ((SelectWhere) countBuilder).where(c.get());
	...
	private <T> UpdateQuery getPatchMany(FindRequest<T> findRequest, Collection<Patch> patchRequest, EntityMeta meta, String tableName, boolean atomic) {
		...
		BuildSelect selectIds = buildSelect(findRequest, meta, tableName, List.of(idColumn), query);
	...
	@Override
	protected Mono<Void> createTable(EntityMeta meta, String tableName, Index[] indexes) {
		Table table = Table.create(tableName);
