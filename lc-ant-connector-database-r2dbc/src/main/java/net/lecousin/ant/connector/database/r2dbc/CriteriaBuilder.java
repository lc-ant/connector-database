...
public abstract class CriteriaBuilder {
	...
	public Optional<Condition> build(ConditionAnd and) {
		...
		for (var c : and.getAnd()) {
			...
			if (result == null) result = Conditions.nest(cd.get());
			else result = result.and(Conditions.nest(cd.get()));
		}
	...
	public Optional<Condition> build(ConditionOr or) {
		...
		for (var c : or.getOr()) {
			...
			if (cd.isEmpty()) continue;
			if (result == null) result = Conditions.nest(cd.get());
			else result = result.or(Conditions.nest(cd.get()));
		}
		return Optional.ofNullable(result);
	}
	...
	public Optional<Condition> build(BinaryOperationExpression<?, ?, Boolean> op) {
		...
		if (op instanceof RegexpMatchExpression) return Optional.of(getCondition(l, r, dialect::conditionRegexpMatch));
